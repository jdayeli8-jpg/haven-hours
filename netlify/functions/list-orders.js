...
export async function handler(event) {
  ...
  if (body.scope === 'recent') {
    const { data, error } = await supabase
      .from('orders')
      .select('id, order_code, stage, name, email, phone, estimate, final_amount, status, fulfillment_status, square_card_id, incident_created_at, incident_decision, ready_created_at, pickup_date, pickup_window, address')
