// netlify/functions/ready-photo.js
//
// TARJETA DIGITAL DE GRACIAS — Adjunta una foto de la ropa YA LISTA (doblada
// y empacada) a una orden real y le manda al cliente un correo cálido con la
// foto y una nota opcional. Espejo de report-incident.js pero en bonito. 💛
//
...
export async function handler(event) {
  ...
  const { data: order, error: updErr } = await supabase
    ...
    .update({
      ...
      ready_token: token,
      ready_created_at: new Date().toISOString(),
    })
    .eq('id', orderId)
  ...
  if (apiKey && order.email) {
    const photoUrl = `${SITE}/api/ready-image?order=${order.id}&token=${token}`
    ...
    const html = `
      <div style="margin:0;background:#F4F1EC;padding:0">
        <div style="font-family:Georgia,'Times New Roman',serif;max-width:520px;margin:0 auto;padding:32px 24px;color:#2A2622;line-height:1.6">
          ...
          <p style="font-size:13px;color:#7a736b;margin:0 0 24px;letter-spacing:.04em;text-transform:uppercase">Your laundry is ready</p>
          <p style="font-size:16px;margin:0 0 14px">Hi ${first},</p>
          <p style="font-size:16px;margin:0 0 18px">Your laundry is freshly cleaned, folded, and packed with care — here's a little peek before it heads back to you.</p>
          <img src="${photoUrl}" alt="Your freshly folded laundry"
          ...
          <p style="font-size:16px;margin:0 0 6px">Every order is washed separately and handled with care — yours included.</p>
          <p style="font-size:16px;margin:0 0 18px">Thank you for trusting us with it. 💛</p>
